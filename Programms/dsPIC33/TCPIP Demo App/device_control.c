...
#ifdef __C30__
    BYTE DMABuffer[2048] __attribute__((space(dma)));
#else
    BYTE DMABuffer[2048];
#endif
...
//------------------------------------------------------------------------------------------------
{
    BYTE* buf = &DMABuffer[BufferBusySize];
    if(BufferBusySize + Count <= 2048){
...
//------------------------------------------------------------------------------------------------
{
    *BufferA = DMAConfig[id].BufA;
    *BufferB = DMAConfig[id].BufB;
    return 0;
}
//------------------------------------------------------------------------------------------------
...
static void WaitForDataReadySPI1( void )
{
    ...
    {
        while ((SPI1STATbits.SPITBF == 0) || (SPI1STATbits.SPIRBF == 0));
    }
}
static void WaitForDataReadySPI2( void )
{
    ...
    {
        while ((SPI2STATbits.SPITBF == 1) || (SPI2STATbits.SPIRBF == 1));
    }
}
#endif
#ifdef _WIN32
...
DEVICE_REG * SPIRegisterDevice(SPI_ID id, SPIConfig Config, int (*DeviceSelect)(void), int (*DeviceRelease)(void))
{
    ...
    Device->id = id;
    Device->SPIStatus = (SPI_STATUS *)&SPIStatus[id];
...
//------------------------------------------------------------------------------------------------
{
    ...
    DEVICE_REG* Device;
    // защита от повтороной инициализации
    if(SPIInitialized) 
        return 0;
    DMAInit();
    ...
    for(i = 0; i < MAX_SPI_DEVICES; i++){
        ...
        Device->id = ID_SPI1;
        Device->SPIStatus = (SPI_STATUS *)&SPIStatus[ID_SPI1];
        Device->OldIntLevel = 0;
        Device->DeviceId = i;
    }
    // SPI2
...
SPIConfig SPI_CreateParams(SPI_MODE Mode, SPI_CLOCK_POLARITY CKP, SPI_CLOCK_EDGE CKE, SPI_CLOCK_PRIMARY_PRESCALER PPRE, SPI_CLOCK_SECONDARY_PRESCALER SPRE,SYS_IDLE Idle,SPI_DATA_SIZE_BIT DataSize, SPI_INPUT_PHASE InputPhase)
{
    ...
    Config.SPICON1 = 0;
    Config.SPICON1 |= ((WORD)PPRE);
    Config.SPICON1 |= ((WORD)SPRE) << 2;
    Config.SPICON1 |= ((WORD)Mode) << 5;
    Config.SPICON1 |= ((WORD)CKP) << 6;
    Config.SPICON1 |= ((WORD)CKE) << 8;
