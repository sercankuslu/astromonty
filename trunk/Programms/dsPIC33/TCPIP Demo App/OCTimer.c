...
int OCInit(void)
{
	...
	// инициализаци€ прерывани€ дл€ заполнени€ буфера
	IFS1bits.U2RXIF = 0;
	...
	//»нициализаци€ порта1
	{
        ...
        // инициализаци€ OC1
    	{
            ...
            IPC0bits.OC1IP = 7;		    // выбрать приоритет прерывани€ дл€ OC1
...
int InitRR(RR * rr)
{
    ...
    rr->B = 0.751428571 / I;
    rr->dx = PI/(180.0*200.0*16.0); // шаг перемещени€ в радианах
    rr->TimerStep = 0.0000002; // шаг таймера
    rr->Vend = 0 * Grad_to_Rad;
    rr->Xend = 1 * Grad_to_Rad; // здесь удвоенна€ координата. т.к. после ускорени€ сразу идет торможение
    rr->DataCount = 0;
    rr->NextWriteTo = 0;
    rr->NextReadFrom = 0;    
    rr->XaccBeg = 0;
