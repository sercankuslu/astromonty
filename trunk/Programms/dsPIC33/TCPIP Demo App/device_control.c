...
//------------------------------------------------------------------------------------------------
INTERRUPT _Interrupt61(void)//_DMA5Interrupt(void) // Ó¯Ë·Í‡ ‚ ÍÓÌÙË„Û‡ˆËË?
//------------------------------------------------------------------------------------------------
{    
    DMA_ID id = DMA5;
    if(DMAConfig[id].fillingBufferBFunc){
...
//------------------------------------------------------------------------------------------------
{
    switch(id){
        case ID_OC1:
            IPC0bits.OC1IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC1
            IFS0bits.OC1IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC0bits.OC1IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC1
            break;
        case ID_OC2:
            IPC1bits.OC2IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC2
            IFS0bits.OC2IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC0bits.OC2IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC2
            break;
        case ID_OC3:
            IPC6bits.OC3IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC3
            IFS1bits.OC3IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC1bits.OC3IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC3
            break;
        case ID_OC4:
            IPC6bits.OC4IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC4
            IFS1bits.OC4IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC1bits.OC4IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC4
            break;
        case ID_OC5:
            IPC10bits.OC5IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC5
            IFS2bits.OC5IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC2bits.OC5IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC5
            break;
        case ID_OC6:
            IPC10bits.OC6IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC6
            IFS2bits.OC6IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC2bits.OC6IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC6
            break;
        case ID_OC7:
            IPC10bits.OC7IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC7
            IFS2bits.OC7IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC2bits.OC7IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC7
            break;
        case ID_OC8:
            IPC11bits.OC8IP = Level;             // –≤—ã–±—Ä–∞—Ç—å –ø—Ä–∏–æ—Ä–∏—Ç–µ—Ç –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –¥–ª—è OC8
            IFS2bits.OC8IF = 0;                 // —Å–±—Ä–æ—Å–∏—Ç—å —Ñ–ª–∞–≥ –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è
            IEC2bits.OC8IE = enabled;           // —Ä–∞–∑—Ä–µ—à–∞–µ–º –ø—Ä–µ—Ä—ã–≤–∞–Ω–∏—è –æ—Ç OC8
            break;
        default:
        return -1;
    }
...
void SPIPPcallBack(void* _This, WORD* DMABuff, WORD BufSize)
{
    ...
    while(!BufferFull){
        ...
        case SEND_DATA2:
            ...
            break;
        case FINAL_PART: // ‚ÒÂ Á‡ÔËÒ‡ÌÓ ‚ ·ÛÙÂ. ÓÒÚ‡ÎÓÒ¸ ÚÓÎ¸ÍÓ ÔÓÏÂÌˇÚ¸ ÂÊËÏ DMA
    ...
    case 1:
        if((DataToSend->Data1Len - DataToSend->Data1Pos) + (DataToSend->Data2Len - DataToSend->Data2Pos) <= BufSize){
            // ÂÒÎË ‰‡ÌÌ˚ı ÓÒÚ‡ÎÓÒ¸ ÏÂÌ¸¯Â, ˜ÂÏ ÌÂÓ·ıÓ‰ËÏÓ ‰Îˇ ÔÓÎÌÓ„Ó ·ÛÙÂ‡
    ...
    case 2:
        // ÓÒÚ‡ÎÓÒ¸ ÓÚÔ‡‚ËÚ¸ Ï‡ÎÂÌ¸ÍËÈ ÍÛÒÓÍ, ÍÓÚÓ˚È ÛÊÂ Á‡„ÛÊÂÌ ‚ ·ÛÙÂ
...
{
    BYTE TmpInt;
    // ÛÒÚ‡ÌÓ‚Í‡ ÚÂÍÛ˘Â„Ó ÔËÓËÚÂÚ‡ CPU Ì‡ 4
    ...
    if(DataLen & 0x0001){
        // ≈ÒÎË ‰ÎËÌÌ‡ ÌÂ˜ÂÚÌ‡ˇ, ÚÓ ÓÚÔ‡‚ËÏ 1 ·‡ÈÚ ÓÚ‰ÂÎ¸ÌÓ ‚ ÍÓÌˆÂ
        DataLen -= 1;
    }
    ...
    // ÓÒ‚Ó·ÓÊ‰ÂÌËÂ ÛÒÚÓÈÒÚ‚‡
    DeviceRelease();
    // ‚ÓÒÒÚ‡ÌÓ‚ÎÂÌËÂ ÔËÓËÚÂÚ‡ CPU
